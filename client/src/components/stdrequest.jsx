...
export default function StdRequest() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:9000/OpDetails")
      .then((result) => {
  ...
  async function acceptOP(rollno) {
    try {
      ...
      if (isOk) {
        const data = await axios.post("http://localhost:9000/acceptRequset", {
          rollno: rollno,
        });
  ...
  async function rejectOP(rollno) {
    try {
      ...
      if (val) {
        const data = await axios.post("http://localhost:9000/rejectRequset", {
          rollno: rollno,
          val: val,
        });
        removeRequest(rollno);
  ...
  async function removeRequest(rollno) {
    console.log(rollno);
    try {
      const data = await axios.post("http://localhost:9000/removeRequset", {
        rollno: rollno,
      });
      // console.log(data);
    } catch (error) {
